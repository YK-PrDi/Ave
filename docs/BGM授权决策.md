...
✅ **2026-09-08 用户已听完确认，12 首全部移回主目录 → 曲库 312 首**，
线上清单已更新（版本 `2026-09-08`）。12 个新 URL 实测全部 200、大小逐首
对得上，端到端下载 `Future_Cha_Cha` **sha256 校验通过**。
...
| 东西 | 位置 |
|---|---|
| 312 首音频 + `来源.json` + `manifest.json` | `D:\bgm-incompetech\`（2.1GB，不进版本库） |
| ~~12 首待听确认~~ ✅ 2026-09-08 已听、已移回主目录 | `待核实\` 现只剩 `来源-待核实.json` |
| 主边车备份（合并 12 条前那份 300 条） | `D:\bgm-incompetech\来源.json.bak-2026-09-08` |
...
2. ~~下载~~ ✅ → ~~上传自有云~~ ✅ 2026-09-07（300 首）+ 2026-09-08（补 12 首）
   → ~~写清单~~ ✅（`host: own`，版本 `2026-09-08`，312 首）
3. ~~数量不够再用 A 补 Pixabay~~ —— **不用了**，312 首已达上限目标
   （第六节原定 100~300 首）。真要补再说，方案不变。

   ⚠️ **上传顺序有硬约束：先传 mp3，最后传 manifest。** 反了的话客户端会拿到
   指向不存在文件的清单，抽中就 404（不崩、如实报错，但那条片子没 BGM）。

   ⚠️ **补曲子必须先合并边车再生成清单。** 那 12 首的元数据原先在
   `待核实\来源-待核实.json`，主边车没有 —— 而 `生成清单.py:179` 按文件名查边车，
   **查不到就用命令行 `--license` 默认值填**，会给 incompetech 的曲子贴上
   `Pixabay Content License`（错误授权）。脚本会报「有 N 首不在边车里」不会静默，
   但结果是错的。**合并对了的判据：重生成时那条警告不出现。**
4. ~~存证~~ ✅ 已归档 `存证/`。
