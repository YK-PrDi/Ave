...
#
# 3.0 是**用户 2026-08-27 试听 0/3/5/8% 四条后亲耳定的**（≈-30dB）。
# 原来是 -18dB（≈12.6%），用户反馈「有点影响口播」。
# 改这个值前先渲几条让用户听 —— 全片均值分不出 3% 和 5% 的差别
# （BGM 比口播低三十分贝，对均值几乎无贡献），客观指标在这里没有裁决权。
BGM_VOLUME = 3.0
...
# ⚠️ **这个值不再决定渲染倍速**（用户 2026-08-26 澄清后改）。
#
# 原来的做法：把它当渲染倍速，画面 setpts 压缩、配音按压缩后的时长贴合。
# 实听反馈「有的念得快有的慢、AI 那段明显听得出来」—— 因为
# `synth_fit()` 是每段各自算语速去填满自己的时长（实测真人 3.84~10.64 字/秒）。
#
# 现在的做法：配音按 `tts.FIXED_SPEED` 固定语速合成，**画面逐段去适配配音时长**
# （`clip_speeds`）。全片语速一致，口播说完画面同步收紧、不留白。
#
# 这个常数现在只剩两个用途：
#   1. 估 AI 文案字数上限（`dur / PLAYBACK_SPEED` 当目标时长）——
#      写得越密，画面适配时倍速越高，等效于「1.2 倍节奏」但语速是自然的
#   2. 前端那个倍速滑块的默认值 —— 那是**播放/导出时的后处理**
